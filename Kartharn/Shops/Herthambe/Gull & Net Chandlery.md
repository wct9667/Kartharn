---
name: Gull & Net Chandlery
type: Shop
location: "[[Herthambe]]"
owner: Halden Reefwalker
faction: Harbor Guild
shop_type: General Store
wealth_level: Low
alignment: Neutral
hours: Sunrise–Sunset
specialties:
  - Rope
  - Nets
  - Lantern oil
reputation: Essential
---

> [!infobox|right]
> # `=this.name`
> ###### Shop Information
> | Type | Stat |
> | ---- | ---- |
> | Shop Type | `=this.shop_type` |
> | Location | `=this.location` |
> | Owner | `=this.owner` |
> | Faction | `=this.faction` |
> | Alignment | `=this.alignment` |
> | Wealth Level | `=this.wealth_level` |
> | Hours | `=this.hours` |
>
> ###### Specialties
> - `=join(this.specialties, ", ")`
>
> ###### Reputation
> `=this.reputation`

## Description
Tar-stained shelves sag under coils of rope and patched nets. The smell of oil never leaves.

## Inventory
### Common Goods
| Item         | Price | Notes               |
| ------------ | ----- | ------------------- |
| Rope (50 ft) | 1 gp  | Salt-soaked         |
| Fishing net  | 5 gp  | Frequently repaired |

### Uncommon / Special Items
| Item              | Price | Requirements  |
| ----------------- | ----- | ------------- |
| [[Storm Lantern]] | 15 gp | Limited stock |

### Rare or Hidden Stock
| Item | Price | Conditions |
| ---- | ----- | ---------- |
| Ever-burning buoy | 75 gp | Harbor Guild |

## Services Offered
- Net repair  
- Gear resale  

## Owner & Staff
### Owner: Halden Reefwalker
- **Personality:** Gruff, practical  
- **Motivation:** Keep docks running  
- **Secret:** Sells salvage quietly  

## Shop Rules & Quirks
- Credit only for locals, prices increased by 50% for non-locals.

## Rumors & Hooks
| Rumor | Truth |
| ----- | ----- |
| Halden fences stolen cargo | True |

## Notes
First stop for any sailor.
